...
--1) Drop command -->by using the DROP TABLE statement with the table name 
-- we can destroy a specific table
-- Syntax: -DROP TABLE <table name>;
Drop table students;

--2)  TRUNCATE COMMAND-->The truncate command is much faster in comparison to 
-- deletestatement but similar to the drop command as to destroy a specific
